...
export class TokenService {
    ...
    async saveToken({ userId, token }: IGeneretedToken) {
        ...
        const tokenDB = await this.prisma.token.findUnique({ where: where, include: { user: true } });
        if (!tokenDB) {
            const userDB = await this.prisma.user.findUnique({ where: { id: userId } })
            const payload = { sub: userDB.id, username: userDB.email, name: userDB.name };
            const data: IToken = {
        ...
        if (tokenDB) {
            ...
            const payload = { sub: tokenDB.user.id, username: tokenDB.user.email, name: tokenDB.user.name };
